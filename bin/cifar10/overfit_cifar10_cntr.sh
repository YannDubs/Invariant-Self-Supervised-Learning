...
if [ "$is_plot_only" = false ] ; then
  for kwargs_dep in  "scheduler@scheduler_issl=plateau,plateau_quick,cosine,cosine_restart"  "architecture@encoder=resnet50" "trainer.max_epochs=100,500" "optimizer_issl.kwargs.lr=3e-4,3e-3" "decodability.kwargs.is_train_temperature=True" "scheduler_issl.kwargs.base.warmup_epochs=3,10" "representor=cntr_stdA,cntr,cntr_1000A,std_cntr"
  do
