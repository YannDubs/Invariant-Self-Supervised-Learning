...
optimizer_issl.kwargs.weight_decay=tag(log,interval(3e-7,1e-5))
scheduler@scheduler_issl=warm_unifmultistep125,warm_unifmultistep100,slowwarm_unifmultistep25,warm_unifmultistep25
...
decodability.kwargs.out_dim=tag(log,int(interval(50,5000)))
decodability.kwargs.beta_pM_unif=tag(log,interval(1e0,2e1))
