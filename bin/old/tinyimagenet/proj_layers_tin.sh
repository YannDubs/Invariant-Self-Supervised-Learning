...
experiment="proj_layers_tin"
notes="
**Goal**: effect of using larger projection heads : number of layers.
"
...
if [ "$is_plot_only" = false ] ; then
  for kwargs_dep in  "decodability.kwargs.projector_kwargs.n_hid_layers=1,2,3,4" "representor=cntr_lin"
  do
...
python utils/aggregate.py \
       ...
       +collect_data.params_to_add.projection_size="decodability.kwargs.projector_kwargs.n_hid_layers" \
       +collect_data.params_to_add_dflt.projection_size=0 \
       ...
       +plot_scatter_lines.filename="acc_vs_loss_projL_w1e-5" \
