...
class DISSLTeacherClf(pl.Callback):
    """Checks if DISSL's teacher recovers the correct labels."""
    def __init__(
        self,
        dm : pl.LightningDataModule,
        plot_interval: int = -1,
        ...
        self.label_names = dm.train_dataset.label_names
        self.train_dataloader = dm.test_dataloader(test_dataset=dm.train_dataset, batch_size=1024, drop_last=False)
        self.test_dataloader = dm.test_dataloader(test_dataset=dm.test_dataset, batch_size=1024, drop_last=False)
        self.plot_interval = plot_interval
    ...
    def on_train_epoch_end(
        ...
        if is_plot(trainer, self.plot_interval):
            ...
            Y=[]
            M_hat=[]
            n_Mx = pl_module.loss_decodability.out_dim

            with torch.no_grad():
                for batch in self.train_dataloader:
                    x, (y,_) = batch
                    z = pl_module(x.to(pl_module.device))
                    m_hat = torch.argmax(pl_module.loss_decodability.get_Mx(z), 1)

                    Y += [to_numpy(y)]
                    M_hat += [to_numpy(m_hat)]

                Y_train = np.concatenate(Y, axis=0)
                M_hat_train = np.concatenate(M_hat, axis=0)

                m_to_y = {i : stats.mode(Y_train[M_hat_train == i])[0] for i in range(n_Mx)}

                self.log_curr_mode(pl_module, trainer, Y_train, M_hat_train, m_to_y, mode="train")
                ...
                M_hat_test = np.concatenate(M_hat, axis=0)

                self.log_curr_mode(pl_module, trainer, Y_test, M_hat_test, m_to_y, mode="test")

            pl_module.train()

    def log_curr_mode(self, pl_module, trainer, Y, M_hat, m_to_y, mode="train"):
        try:
            Y_hat = np.concatenate([m_to_y[m] for m in M_hat])

            acc = (Y == Y_hat).mean()
            logger.info(f"DISSL Teacher: {mode} acc={acc}")
            pl_module.log(f"{mode}/{pl_module.stage}/{pl_module.hparams.task}/Mx_acc", acc, on_epoch=True)

            conf_mat = pd.DataFrame(confusion_matrix(Y, M_hat, labels=range(len(self.label_names))),  index=self.label_names)
            conf_mat_wandb = wandb.Table(data=conf_mat.T)
            experiment = trainer.logger.experiment
            ...
            logger.info(f"Error with Y_hat_{mode}={Y_hat}, Y_{mode}={Y}")
